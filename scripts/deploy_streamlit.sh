...
snowsql -a "$SNOWSQL_ACCOUNT" -u "$SNOWSQL_USER" -r "$SNOWSQL_ROLE" -w "$SNOWSQL_WAREHOUSE" -d "$SNOWSQL_DATABASE" -q "
  USE SCHEMA ${SNOWSQL_DATABASE}.${SNOWSQL_EDW_SCHEMA};
  CREATE OR REPLACE STAGE ${APP_STAGE};
"
snowsql -a "$SNOWSQL_ACCOUNT" -u "$SNOWSQL_USER" -r "$SNOWSQL_ROLE" -w "$SNOWSQL_WAREHOUSE" -d "$SNOWSQL_DATABASE" -q "PUT file://${root}/streamlit/app.py @${SNOWSQL_DATABASE}.${SNOWSQL_EDW_SCHEMA}.${APP_STAGE} AUTO_COMPRESS=FALSE OVERWRITE=TRUE;"

echo "Creating Streamlit app ${APP_NAME} ..."
snowsql -a "$SNOWSQL_ACCOUNT" -u "$SNOWSQL_USER" -r "$SNOWSQL_ROLE" -w "$SNOWSQL_WAREHOUSE" -d "$SNOWSQL_DATABASE" -q "
  CREATE OR REPLACE STREAMLIT ${APP_NAME}
    FROM @${SNOWSQL_DATABASE}.${SNOWSQL_EDW_SCHEMA}.${APP_STAGE}
    MAIN_FILE='app.py'
    QUERY_WAREHOUSE='${SNOWSQL_WAREHOUSE}';
  SELECT SYSTEM$SHOW_STREAMLIT_URL('${APP_NAME}') AS url;
"
